...
import { AlertTriangle, Phone, Navigation, X, MapPin, Star, ExternalLink, Loader2 } from 'lucide-react';
...
const EmergencyMapChild = () => {
  ...
  useEffect(() => {
    ...
    providers.forEach(p => {
      ...
      if (em.has(p.id)) { em.get(p.id)!.setIcon(createMarkerIcon(p.type, sel, true)); }
      else {
        const m = L.marker([p.lat, p.lng], { icon: createMarkerIcon(p.type, sel, true) });
  ...
  useEffect(() => {
    return () => { if (markerGroupRef.current) { removeMarkerLayer('emergency'); markerGroupRef.current = null; markersMapRef.current.clear(); } };
  }, [removeMarkerLayer]);
